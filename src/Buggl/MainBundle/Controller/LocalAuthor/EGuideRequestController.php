...
class EGuideRequestController extends Controller 
{
    ...
    public function eGuideRequestSubmitAction(Request $request)
    {
        ...
        $tempPath = 'uploads/requestplan_temp';
      ...
      // print_r($params); die;
        ...
        if($file !=''){
        $eguidereq->setUserfile($filename);}
		...
		//\Stripe\Stripe::setApiKey('sk_test_TzdpwBfX06VBWo6Cj3ctNwSm');
        ...
        try {
           ...
           // echo $em->getSql();
            ...
            $em->flush();
             $eguidereq->getId();
            $filepath = 'http://'.$request->getHost().'/uploads/requestplan_temp';
            $this->get('buggl_main.eguide_request_service')->sendEGuideRequest($eguidereq,$email,$useremail,$userip,$destination,$params['duration'],$country1,$filepath);
