(function($){
    $.gallery = function( options ){
        ...
        window.UploadView = Backbone.View.extend({
            el: $('div#preview'),
            initialize: function(){
                _.bindAll(this,'render','removeLoader');
        ...
        window.ImageView = Backbone.View.extend({
            ...
            delete: function(e){
                ...
                $.fn.bugglConfirm({
                    'onConfirm':function(){
                        $("span.loader").show().prevAll().hide();
                        $.post(options.deleteUrl,{photos:id},function(data){
                            if(data.success==true){
                                $.fn.bugglAlert({
                                    'title' : "Deleted Photo",
                                    'content' : '<p>successfully deleted</p>',
                                    'revealOptions' : {
                                        'closeOnBackgroundClick':false,
                                        'closed':function(){
                                            // self.parents('li.column').remove();
                                            // $('span.loader').hide().prevAll().show();
                                            window.location.reload();
                                        }
                                    }
                                });
        ...
        window.ThumbnailView = Backbone.View.extend({
            // el: $('#thumbnail'),
            initialize: function(){
                _.bindAll(this,'render','renderImages','fetchImages','empty');
