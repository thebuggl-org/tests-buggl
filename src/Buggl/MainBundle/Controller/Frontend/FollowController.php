...
class FollowController extends Controller implements \Buggl\MainBundle\Interfaces\BugglSecuredPage
{
	...
	public function followedAction(Request $request)
	{
		...
		if(!is_null($followed))
		{
			$following = json_decode( $followed->getFollowing(), true );

			$totalFollowed = count($following);

			$following = array_slice( $following, $offset, $limit );

			$authors = array();
			foreach($following as $followed){
		 		$obj = $this->getDoctrine()->getEntityManager()->getRepository('BugglMainBundle:LocalAuthor')->findOneById($followed);
				$authors[] = $obj;
			}
			$countedFollowing = $totalFollowed;
			...
			if($page == 1)
			{
				...
				$data = array(
					'content' => $this->renderView('BugglMainBundle:LocalAuthor\Dashboard:followedModalPart.html.twig', array('authors' => $authors, 'isFollowing' => 1,'hasNext' => $hasNext,'text' => 1,'countedFollowing' => $countedFollowing ))
				);
			}
