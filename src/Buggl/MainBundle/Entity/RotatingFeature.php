...
class RotatingFeature
{
	...
	public function getImageAbsolutePath($orig = false)
    {
        ...
        if(null === $this->photo)
            ...
            if($orig){
                if( file_exists($this->getUploadRootDir().'/orig_'.$this->photo) )
                    return $this->getUploadRootDir().'/orig_'.$this->photo;
            }
    ...
    public function getImageWebPath($orig = false)
    {
        if(null === $this->photo)
            return null;
        else {
            if($orig){
                if( file_exists($this->getUploadRootDir().'/orig_'.$this->photo) )
                    return $this->getUploadDir().'/orig_'.$this->photo;
            }
    ...
    public function getWidth($orig = false)
    {
        $file = $this->getImageAbsolutePath($orig);
        if(file_exists($file)){
            $list = getimagesize($file);
            return $list[0];
        }
        return 0;
    ...
    public function getHeight($orig = false)
    {
        $file = $this->getImageAbsolutePath($orig);
        if(file_exists($file)){
            $list = getimagesize($file);
