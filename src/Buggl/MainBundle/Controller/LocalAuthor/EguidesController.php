...
class EGuidesController extends Controller implements \Buggl\MainBundle\Interfaces\BugglSecuredPage
{
	...
	$this->eguide = (!$guide) ? new EGuide() : $guide;
		...
		{  
		 $this->get('session')->setFlash('success','Your Eguide is completed you can now published!');
			...
			$itsd = $this->getDoctrine()->getRepository('BugglMainBundle:EGuideToSpotDetail')->getByRequestEGuide($guide);
            $abc=$itsd->getSpotDetail();
           //echo $abc;
            $em = $this->getDoctrine()->getEntityManager();
            $sd = $this->getDoctrine()->getRepository('BugglMainBundle:SpotDetail')->findOneById($abc);
			$egtsd = $this->getDoctrine()->getRepository('BugglMainBundle:EGuideToSpotDetail')->findOneBy(array('eGuide' => $guide, 'spotDetail' => $sd));
			$egtsd->setIsFeatured(1);
			$em->persist($egtsd);
			$em->flush();
			$result = $this->get('buggl_main.pdf_creator')->generate($guide);
			...
			$paymentsService->saveRequestPurchase($guide,$buyer);
	...
	private function finishTravelGuide(Request $request)
	{
		...
		{
			$sdID = $iObj->getSpotDetail()->getId(); echo $sdID;
			//die($sdID);
